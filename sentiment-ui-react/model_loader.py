...
def _ollama_post(path: str, payload: dict) -> dict:
    """Try each Ollama host until one works."""
    last_err = None
...
def generate_opinion(text: str) -> dict:
    """Predict sentiment then generate a one-shot analytical opinion via Ollama."""
    result = predict(text)
    label      = result["label"]
...
def chat(history: list) -> dict:
    """
    Multi-turn conversation via Ollama /api/chat.

    `history` is a list of {"role": ..., "content": ...} dicts
    that includes the system prompt and full conversation so far.
    Returns {"reply": str}.
    """
